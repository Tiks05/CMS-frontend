<template>
  <div class="index_booklist">
    <div class="booklist_con" ref="container" @mousemove="handleMouseMove" :style="contentStyle">
      <!-- 用 v-for="6" 创建 6 列；奇数列增加 .dong_list_on 类向上偏移（错位）；每列内部又 v-for="3" 显示 3 本图书；用 i * 3 + j 精确定位书籍。 -->
      <div
        class="dong_list"
        v-for="(i, iIndex) in 6"
        :key="iIndex"
        :class="i % 2 == 0 ? '' : 'dong_list_on'"
      >
...
const handleMouseMove = (e: MouseEvent) => {
  ...
  const rect = container.value.getBoundingClientRect() // 获取 container 元素在 视口中的位置信息和尺寸，返回的是一个 DOMRect 对象
  mouseX.value = (e.clientX - rect.left) / rect.width // e.clientX 是鼠标相对于视口的水平位置，rect.left 是 container 元素左边界相对于视口的水平位置
  mouseY.value = (e.clientY - rect.top) / rect.height // e.clientY 是鼠标相对于视口的垂直位置，rect.top 是 container 元素上边界相对于视口的垂直位置
}
</script>
