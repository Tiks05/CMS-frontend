<template>
	...
	<el-form :model="form" :rules="rules" ref="formRef" class="login-form">
		...
		<el-form-item prop="code">
			<div class="code-line">
				<el-input v-model="form.code" placeholder="请输入验证码" size="large" class="input-rounded code-input" />
		...
		<!-- 登录按钮 -->
		<el-button :class="['login-btn', { active: canSubmit }]" :style="{
			cursor: canSubmit ? 'pointer' : 'not-allowed',
			pointerEvents: canSubmit ? 'auto' : 'none'
		}" :disabled="false" @click="canSubmit ? handleLogin() : null">
			登录/注册
		</el-button>
...
const canSubmit = computed(() =>
	isPhoneValid.value && form.code.trim() !== '' && form.agree
)

const canSendCode = computed(() =>
	isPhoneValid.value && form.agree && countdown.value === 0
)
