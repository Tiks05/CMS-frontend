  ...
  <div class="home-activity">
    <div class="serial-card">
      <div class="home-activity-title">
        <h4>原创新书榜<em>统计时间：{{ yesterdayDate }} 24:00</em></h4>
        <span class="author-title-extra">
          <span class="home-activity-title-more">查看全部
            <!-- icon 省略不动 -->
          </span>
        </span>
      </div>
      ...
      <div class="word-list-switch">
        <div class="down_btn">
          ...
          <div class="down_link">
            <em v-for="(cat, i) in categoryList" :key="i" @click="activeCategory = cat">{{ cat }}</em>
          </div>
        </div>
      ...
      <div class="home-new-book-list">
        <div class="hot-card" v-for="(item, i) in currentList" :key="i" @click="goTo(item.path)">
          <div
            class="hot-card-rank"
            :class="i === 0 ? 'hot-card-rank-gold1' : i === 1 ? 'hot-card-rank-gold2' : i === 2 ? 'hot-card-rank-gold3' : ''"
          >
...
<style scoped>
	...
	.hot-card-right-cover {
		...
		.hot-card-right-cover img{width: 42px;height: 56px;
